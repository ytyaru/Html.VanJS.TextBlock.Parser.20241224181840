...
　インライン要素でコードをハイライトしたい場合もある。\`\`md \`markdown\` \`\`こんな感じ。
　インライン要素でコードをハイライトしたい場合もある。\`md # マークダウンの表題記法 \`こんな感じ。
　インライン要素でコードをハイライトしたい場合もある。\`js alert('Hello JS !!'); \`こんな感じ。
　インライン要素でコードをハイライトしたい場合もある。\`\`js alert(\`Hello JS !!\`); \`\`こんな感じ。
　インライン要素でコードをハイライトしたい場合もある。\`\`md \`js  alert('JS'); \` \`\`こんな感じ。
　インライン要素でコードをハイライトしたい場合もある。\`\`\`md \`\`js  alert(\`JS\`); \`\` \`\`\`こんな感じ。
...
---url MDN.HTML https://developer.mozilla.org/ja/docs/Web/HTML/Element/ text=\`html <{id}>\`
a	a	"<a>: アンカー要素"
/(h[1-6]|heading)/	Heading_Elements	"<h1>–<h6>: HTML の見出し要素"
/heading/	Heading_Elements	"<h1>–<h6>: HTML の見出し要素"	\`html <h1>\`〜\`html <h6>\`
---
